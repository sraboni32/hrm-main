                ...
                <div class="card-body">
                    <div class="mb-4">
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
                            <h3 class="mb-2 mb-md-0">{{ $task->title }}</h3>
                            <div class="d-flex gap-2">
                                @can('task_edit')
                                <a href="/tasks/{{ $task->id }}/edit" class="btn btn-success btn-sm"
                                   data-toggle="tooltip" data-placement="top" title="{{ __('translate.Edit_Task') }}">
                                    <i class="i-Edit mr-1"></i>
                                    <span class="d-none d-sm-inline">{{ __('translate.Edit') }}</span>
                                </a>
                                @endcan

                                <a href="/tasks" class="btn btn-secondary btn-sm"
                                   data-toggle="tooltip" data-placement="top" title="{{ __('translate.Back_to_List') }}">
                                    <i class="i-Arrow-Left mr-1"></i>
                                    <span class="d-none d-sm-inline">{{ __('translate.Back') }}</span>
                                </a>
                            </div>
                        </div>
                        ...
                        <p class="mt-3"><strong>{{ __('translate.Summary') }}:</strong></p>
                        <div class="border rounded p-3 bg-light">{{ $task->summary ?? __('No summary provided.') }}</div>
                    </div>
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            <a class="nav-item nav-link" id="nav-links-tab" data-toggle="tab" href="#nav-links"
                                role="tab" aria-controls="nav-links"
                                aria-selected="false">{{ __('translate.Links') }}</a>
                        ...
                        <div class="tab-pane fade" id="nav-links" role="tabpanel"
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="text-left">
                                        <div class="text-left bg-transparent">
                                            <a class="btn btn-primary btn-md m-2" @click="New_Link"><i
                                                    class="i-Add text-white mr-2"></i>
                                                {{ __('translate.Add_Link') }}</a>
                                        </div>
                                        <div class="table-responsive">
                                            <table id="ul-contact-list" class="display table data_datatable"
                                               >
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="{{$link->url}}" target="_blank" class="text-primary">
                                                                <i class="i-External-Link mr-1"></i>
                                    ...
                                    <!-- Modal Add Link -->
                                    <div class="modal fade" id="Link_Modal" tabindex="-1" role="dialog"
                                        aria-labelledby="Link_Modal" aria-hidden="true">
                                        <div class="modal-dialog modal-md" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title">{{ __('translate.Add_Link') }}</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <form @submit.prevent="Create_Link()">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-12">
                                                                <label for="link_title"
                                                                    class="ul-form__label">{{ __('translate.Link_Title') }}
                                                                    <span class="field_required">*</span></label>
                                                                <input type="text" v-model="link.title"
                                                                    class="form-control" name="link_title" id="link_title"
                                                                    placeholder="{{ __('translate.Enter_Link_Title') }}">
                                                                <span class="error"
                                                                    v-if="errors_link && errors_link.title">
                                                                    @{{ errors_link.title[0] }}
                                                                </span>
                                                            </div>
                                                            ...
                                                            <div class="col-md-12">
                                                                <label for="link_url"
                                                                    class="ul-form__label">{{ __('translate.URL') }}
                                                                    <span class="field_required">*</span></label>
                                                                <input type="url" v-model="link.url"
                                                                    class="form-control" name="link_url" id="link_url"
                                                                    placeholder="{{ __('translate.Enter_URL') }}">
                                                                <span class="error"
                                                                    v-if="errors_link && errors_link.url">
                                                                    @{{ errors_link.url[0] }}
                                                                </span>
                                                            </div>

                                                            <div class="col-md-12">
                                                                <label for="link_description" class="ul-form__label">
                                                                    {{ __('translate.Description') }}
                                                                </label>
                                                                <textarea v-model="link.description"
                                                                    class="form-control" name="link_description"
                                                                    id="link_description"
                                                                    placeholder="{{ __('translate.Please_provide_any_details') }}"></textarea>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row mt-3">
                                                            <div class="col-md-6">
                                                                <button type="submit" class="btn btn-primary"
                                                                    :disabled="Submit_Processing_link">
                                                                    {{ __('translate.Submit') }}
                                                                </button>
                                                                <div v-once class="typo__p"
                                                                    v-if="Submit_Processing_link">
                                                                    <div class="spinner spinner-primary mt-3"></div>
                                                                </div>
                                                            </div>
                                                        </div>
...
//--------------------------------------------- Links-----------------------------------------------------------\\
            ...
            //------------------------------ Show Modal (Create Link) -------------------------------\\
            New_Link() {
                this.Reset_Form_Link();
             ...
             Create_Link() {
                var self = this;
                self.Submit_Processing_link = true;
                axios.post("/task_links", {
                    title: self.link.title,
                    url: self.link.url,
                    description: self.link.description,
                    task_id: self.task.id,
                }).then(response => {
                        self.Submit_Processing_link = false;
                        window.location.href = '/tasks/'+ self.task.id;
                        toastr.success('{{ __('translate.Created_in_successfully') }}');
                        self.errors_link = {};
                })
                .catch(error => {
                    self.Submit_Processing_link = false;
                    if (error.response.status == 422) {
                        self.errors_link = error.response.data.errors;
                    }
                    toastr.error('{{ __('translate.There_was_something_wronge') }}');
                });
            },
