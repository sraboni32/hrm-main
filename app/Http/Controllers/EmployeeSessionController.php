...
class EmployeeSessionController extends Controller
{
    ...
    {
        // Get current Bangladesh time for attendance
        $bangladesh_now = Carbon::now('Asia/Dhaka');
        $current_day = $bangladesh_now->format('Y-m-d');
        ...
        try{
            if ($punch_in_str === 'Flexible time' || $punch_out_str === 'Flexible time') {
                // Use Bangladesh time for flexible shifts
                $punch_in = $punch_out = $time_now = new DateTime($bangladesh_now->format('Y-m-d H:i:s'));
            } else {
                $punch_in  = new DateTime(substr($punch_in_str, 0, -2));
                $punch_out  = new DateTime(substr($punch_out_str, 0, -2));
                // Use Bangladesh time for current time
                $time_now = new DateTime($bangladesh_now->format('Y-m-d H:i:s'));
            }
        }catch(Exception $e){
            return $e;
        }
